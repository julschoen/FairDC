...
''' ConvNet '''
class ConvNetEven(nn.Module):
    def __init__(self, channel, num_classes, net_width, net_depth, net_act, net_norm, net_pooling, im_size = (32,32)):
        super(ConvNetEven, self).__init__()
    ...
    def _make_layers(self, channel, net_width, net_depth, net_norm, net_act, net_pooling, im_size):
        ...
        shape_feat = [in_channels, im_size[0], im_size[1]]
        for d in range(net_depth):
            padding = 1  # Adjusted padding for kernel size 4
            layers += [nn.Conv2d(in_channels, net_width, kernel_size=4, padding=padding)]
            shape_feat[0] = net_width
            if net_norm != 'none':
                layers += [self._get_normlayer(net_norm, shape_feat)]
            layers += [self._get_activation(net_act)]
...
class LeNet(nn.Module):
    def __init__(self, channel, num_classes, im_size=(32, 32)):
        ...
        if im_size[0] == 28:
            ...
            num_pooling_layers = np.log2(im_size[0])-3
...
### VIT From: https://github.com/kentaroy47/vision-transformers-cifar10
# helpers
